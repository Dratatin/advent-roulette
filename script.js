...
(function () {
  ...
  const TEST_MODE = true; // Change to true to enable test mode
  ...
  async function fetchDescriptions() {
    try {
      ...
      alert("Erreur lors du chargement des descriptions. Veuillez réessayer.");
      return false;
    }
  }
  ...
  async function checkAuth() {
    ...
    const enteredPassword = prompt("⚡Le moooot de passe ? (Harry-Potter)");

    if (enteredPassword === null) {
      document.body.innerHTML =
        '<div style="text-align:center;padding:50px;color:#fff;"><h1>Accès Refusé</h1><p>Mot de passe requis pour accéder à cette page.</p></div>';
      return false;
    }
  ...
  function updateTimer() {
    ...
    if (timeLeft < 0) {
      message.textContent = "🎄 Noyeux Joël ! 🎄";
    ...
    if (!hasAdventStarted()) {
      message.textContent = `⏰ Bientôt bientôôôôt ${formatCountdown(
        timeLeft
      )}`;
      spinButton.disabled = true;
  ...
  async function init() {
    ...
    message.textContent = "⏳ Chargement des descriptions...";
    spinButton.disabled = true;
    ...
    if (!descriptionsLoaded) {
      message.textContent = "❌ Erreur de chargement. Rechargez la page.";
      return;
    }
  ...
  function updateButtonState() {
    if (TEST_MODE) {
      ...
      const testIndicator = "🧪 MODE TEST | ";
      if (state.remainingNumbers.length === 0) {
        spinButton.disabled = true;
        message.textContent = testIndicator + "🎉 C'est fini :(";
      } else {
        spinButton.disabled = false;
        message.textContent =
          testIndicator + `${state.remainingNumbers.length} numéros restants.`;
      }
      if (timerInterval) {
        clearInterval(timerInterval);
      }
    ...
    if (!isAdventPeriod()) {
      ...
      if (!hasAdventStarted()) {
        message.textContent =
          "⏰ Le calendrier de l'avent commence bientôt hop hop !";
      } else {
        message.textContent = "🎄 Noyeux Joël ! 🎄";
      }
    } else if (state.remainingNumbers.length === 0) {
      spinButton.disabled = true;
      message.textContent = "🎉 C'est fini :(";
      if (timerInterval) {
        clearInterval(timerInterval);
      }
    } else if (hasDrawnToday()) {
  ...
  function drawNumber() {
    ...
    animateSpin(() => {
      ...
      message.textContent = `🎉 Le numéro du jour est : ${drawnNumber} !`;
  ...
  async function resetCalendar() {
    const enteredPassword = prompt(
      "Entrez le mot de passe pour réinitialiser le calendrier :"
    );
    ...
    if (hashedInput === PASSWORD_HASH) {
      if (
        confirm(
          "Êtes-vous sûr de vouloir réinitialiser le calendrier ? Toute la progression sera perdue."
        )
      ) {
        resetState();
